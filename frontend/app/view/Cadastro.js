/*
 * File: app/view/Cadastro.js
 *
 * This file was generated by Sencha Architect version 4.2.5.
 * http://www.sencha.com/products/architect/
 *
 * This file requires use of the Ext JS 6.7.x Classic library, under independent license.
 * License of Sencha Architect does not include license for Ext JS 6.7.x Classic. For more
 * details see http://www.sencha.com/license or contact license@sencha.com.
 *
 * This file will be auto-generated each and everytime you save your project.
 *
 * Do NOT hand edit this file.
 */

Ext.define('NET.view.Cadastro', {
    extend: 'Ext.panel.Panel',
    alias: 'widget.cadastro',

    requires: [
        'NET.view.CadastroViewModel',
        'NET.view.CadastroViewController',
        'Ext.container.Container',
        'Ext.button.Button'
    ],

    controller: 'cadastro',
    viewModel: {
        type: 'cadastro'
    },
    height: 250,
    width: 400,
    title: 'Cadastro',

    items: [
        {
            xtype: 'container',
            padding: 5,
            items: [
                {
                    xtype: 'button',
                    text: 'Novo provedor',
                    listeners: {
                        tap: 'onNewProviderTap'
                    }
                }
            ]
        },
        {
            xtype: 'container',
            padding: 5,
            items: [
                {
                    xtype: 'button',
                    text: 'Novo cliente',
                    listeners: {
                        tap: 'onNewClientTap'
                    }
                }
            ]
        },
        {
            xtype: 'container',
            padding: 5,
            items: [
                {
                    xtype: 'button',
                    text: 'Novo plano',
                    listeners: {
                        tap: 'onNewPlanTap'
                    }
                }
            ]
        }
    ]

});